...
-- all c : Component | all p : c.parts | p in c.cposition.prevs.parts --incorrect 1 
 all c : Component | all p : c.parts | some pos : c.cposition.prevs | p.cposition = pos-- all c: Component | ((c.^parts) & Component).cposition not in (c.cposition).*next --incorrect 3

...
--  all u : User | some w : Work | u->w in profile implies (w.source in Institution or w.source in User)  --incorrect 30
  all u : User | some w : Work | u->w in profile implies some (w<:source).Institution or some (w<:source).User --  all u:User , w:Work, i:Institution | w in u.profile && (w.source = u)  --incorrect 32

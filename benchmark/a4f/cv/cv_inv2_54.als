...
--  all u:User, w:(u.profile + u.visible) | w in (u+Institution)  --incorrect 53
  all u:User , w:Work | (w in u.profile) implies (w.source = u || w.source = Institution)  --  all u : User | Work.(u<:profile) in (Work<:source).u + (Work<:source).Institution  --incorrect 55

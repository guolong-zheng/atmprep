...
-- all s:Student,c:Class | some t:Teacher | some s.(c.Groups) and some t.(c.Groups)  --incorrect 136
 all s:Student,c:Class | some t:Teacher | some s.(c.Groups) and some t.(c.Groups) implies t in s.^~Tutors -- all s:Student,c:Class | some t:Teacher | t->s in Tutors  --incorrect 138

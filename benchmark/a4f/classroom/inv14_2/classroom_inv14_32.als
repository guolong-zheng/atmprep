...
-- all c:Class,s:Student,t:Teacher | some s.(c.Groups) and some t.(c.Groups)  --incorrect 31
 all c:Class,s:Student,t:Teacher | some s.(c.Groups) and some t.(c.Groups) and t->c in Teaches implies t in s.^~Tutors -- all c:Class,s:Student,t:Teacher | some s.(c.Groups) and some t.(c.Groups) implies t in c.^~Tutors  --incorrect 33

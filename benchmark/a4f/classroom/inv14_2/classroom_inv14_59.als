...
-- all s : Person, c : Class, t : Teacher | ((some g : Group | c->s->g in Groups) and t->c in Teaches) implies t->s in Tutors  --incorrect 58
 all s : Person, c : Class, t : Teacher | (all g : Group | c->s->g in Groups) and t->c in Teaches implies t->s in Tutors -- all s : Student |   some c : Class | (s in c.Groups.Group) implies (        all t : Teaches.c :> Teacher | s in t.Tutors        )  --incorrect 60

...
-- all s:Student| some t:Teacher | t->s in Tutors  --incorrect 159
 all s:Student| some t:Teacher,c:Class,g:Group | t->s in Tutors and c->s->g in Groups and c->t->g in Groups -- all t : Teacher, c : Class | some Student.(c.Groups) and some (t<:Teaches).c implies some (t<:Tutors).Student  --incorrect 161

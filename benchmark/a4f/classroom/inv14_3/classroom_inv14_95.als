...
-- all s : Student, c : Class | some s.(c.Groups) and some (Teacher<:Teaches).c implies some (Teacher<:Tutors).s  --incorrect 94
 all s : Student, c : Class | some s.(Teacher<:Tutors) implies some c.(Teacher<:Teaches) -- all s : Student, c : Class | some t : Teacher | (some g : Group | c->s->g in Groups) and t->c in Teaches implies t->s in Tutors  --incorrect 96

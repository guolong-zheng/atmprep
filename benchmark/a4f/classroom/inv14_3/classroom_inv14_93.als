...
-- all s : Student, c : Class | some c.(Teacher<:Teaches) implies some s.(Teacher<:Tutors)  --incorrect 92
 all s : Student, c : Class | some g : Group | c->s->g in Groups implies some t : Teacher | t->s in Tutors -- all s : Student, c : Class | some s.(c.Groups) and some (Teacher<:Teaches).c implies some (Teacher<:Tutors).s  --incorrect 94

...
-- all s : Student, c : Class | some g : Group | c->s->g in Groups implies some t : Teacher | t->s in Tutors  --incorrect 93
 all s : Student, c : Class | some s.(c.Groups) and some (Teacher<:Teaches).c implies some (Teacher<:Tutors).s -- all s : Student, c : Class | some s.(Teacher<:Tutors) implies some c.(Teacher<:Teaches)  --incorrect 95

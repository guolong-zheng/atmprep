...
-- all s:Student,c:Class | some s.(c.Groups) implies some t:Teacher | t->c in Teaches  --incorrect 132
 all s:Student,c:Class | some s.(c.Groups) implies some t:Teacher | t->c in Teaches and t in s.~Tutors -- all s:Student,c:Class | some t:Teacher | some c.Groups.s  --incorrect 134

...
-- all s:Student,c:Class | some s.(c.Groups)  --incorrect 131
 all s:Student,c:Class | some s.(c.Groups) implies some t:Teacher | t->c in Teaches -- all s:Student,c:Class | some s.(c.Groups) implies some t:Teacher | t->c in Teaches and t in s.~Tutors  --incorrect 133

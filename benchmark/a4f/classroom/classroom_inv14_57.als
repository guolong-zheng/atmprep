...
-- all p : Student | some c : Class, g : Group | c->p->g in Groups implies some t : Teacher | t->p in Tutors and t->c in Teaches  --incorrect 56
 all s : Person, c : Class | some s.(Teacher<:Tutors) implies some c.(Teacher<:Teaches) -- all s : Person, c : Class, t : Teacher | ((some g : Group | c->s->g in Groups) and t->c in Teaches) implies t->s in Tutors  --incorrect 58

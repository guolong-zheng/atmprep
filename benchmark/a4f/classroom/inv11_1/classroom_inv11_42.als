...
-- all c : Class | some t : Teacher | (t -> c not in Teaches) implies not (some p : Person, g : Group | c -> p -> g in Groups)  --incorrect 41
 all c : Class | some t : Teacher | some s : Student | some g : Group | c->s->g in Groups and t->c in Teaches -- all c : Class | some t : Teacher | some s : Student | some g : Group | c->s->g in Groups implies t->c in Teaches  --incorrect 43

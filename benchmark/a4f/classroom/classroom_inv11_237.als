...
-- all x : Class | some y : Teacher | some g : Group | x->y->g in Groups  --incorrect 236
 not all c : Class, t : Teacher | (t -> c not in Teaches) implies (all p : Person, g : Group | c -> p -> g not in Groups) -- some c : Class | (some p : Person | some g : Group | c->p->g in Groups) implies some t : Teacher | t->c in Teaches  --incorrect 238

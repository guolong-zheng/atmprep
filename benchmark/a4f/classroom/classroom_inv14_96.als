...
-- all s : Student, c : Class | some s.(Teacher<:Tutors) implies some c.(Teacher<:Teaches)  --incorrect 95
 all s : Student, c : Class | some t : Teacher | (some g : Group | c->s->g in Groups) and t->c in Teaches implies t->s in Tutors -- all s : Student, c : Class, g : Group | c->s->g in Groups implies some t : Teacher | t->s in Tutors  --incorrect 97

...
-- all s : Student |   some c : Class | (s in c.Groups.Group) implies (        all t : Teaches.c :> Teacher | s in t.Tutors        )  --incorrect 60
 all s : Student | (some c : Class, g : Group | c->s->g  in Groups) implies some t : Teacher | t->s in Tutors -- all s : Student | (some c : Class, g : Group | c->s->g in Groups) implies some t : Teacher | t->s in Tutors  --incorrect 62

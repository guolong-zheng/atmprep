...
fact { some Employee and some Researcher
all p : Employee | some r : Room | CanEnter[p, r]
}

fact {
  all r : Room |some opened_by.r
  all e : Employee |some k : Key | k in e.owns and secure_lab != k.opened_by
}
...
pred repair_pred_1 {
  some p : Person | CanEnter[p, secure_lab]
  all p : Person | CanEnter[p, secure_lab] implies p in Researcher
}
 run repair_pred_1

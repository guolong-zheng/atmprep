...
/*
 * Model of the Java holeType system. The TypeSoundness assertion
 * claims that if a Java program holeType checks successfully,
 * then a field will cannot be assigned an incorrect holeType.
 *
...
sig State {
  ...
  all e: Expr | let v = val[e] {
    ...
    e in Constructor => v in Object and v.holeType = e.holeType }
  }

pred RuntimeTypesOK [s: State] {
  all o: s.objects, f: o.holeType.fields |
...
pred HasType [v: Value, t: Type] {
  v in Null or Subtype [v.holeType, t]
  }
...
pred TypeChecksSetter [stmt: Setter] {
  all g: Getter & stmt.(lexpr+rexpr).subexprs | g.field in g.expr.holeType.fields
  stmt.field in stmt.lexpr.holeType.fields
  Subtype [stmt.rexpr.holeType, stmt.field.declType]
  }
